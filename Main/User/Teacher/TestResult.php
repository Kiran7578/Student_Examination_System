                ...
                $tscore=$result*$correct-$wrong*$mistake;
                 //insertion in history
                    $username=$data['Teacher_Name'];
                     $studentdept=$data['Teacher_Dept'];
                    $studentteacher=$data['Teacher_Name'];
               $chck="select * from history where username='$username' AND eid='$eid' ";
               $rslt=$db->query($chck);
               if($rslt->num_rows==0)
               {
                $sql="insert into history values ('','$username','".$data['Teacher_Email']."','".$data['Teacher_Email']."','$eid','0','0','0','$tscore',now(),'0','".$studentdept."','$studentteacher')";
                mysqli_query($db,$sql);      
             }
              $ins="update history set totals='$n',correct='$result',wrong='$wrong',score='$tscore',date=now(),Test_Teacher='$studentteacher' where username='$username' AND eid='$eid' ";
               mysqli_query($db,$ins);
